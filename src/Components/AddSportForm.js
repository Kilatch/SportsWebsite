  ...
  addSportToApi(SpName) {
    api
      ...
      .then((res) => {
        this.setState({
          savedSportNameAndMessage:
            res.data.name + ' was successfully added to the database',
        })
      })
      .catch((error) => {
        this.setState({
  ...
  handleSubmit = () => {
    ...
    for (let i = 0; i < this.state.sportList.length; i++) {
      ...
      if (sp.name === this.state.formData.sportName) {
        ...
        this.setState({
          savedSportNameAndMessage:
            'Similar Sport Name Already Exists in The Database And This Might Create confussion!',
        })
      }
    }
      ...
      <div>
        <div>
          <Typography className={classes.txt}>
            {this.state.savedSportNameAndMessage}
          </Typography>
          <ValidatorForm
            ref="form"
