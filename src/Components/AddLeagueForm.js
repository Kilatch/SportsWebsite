...
class AddLeagueForm extends React.Component {
  ...
  handleDialog(saveOk) {
    ...
    if (saveOk == 1) {
      this.addLeagueToApi(
        this.state.formData.leagueName,
        this.getSportId(this.state.formData.sportName)
      )

      console.log(
        'submited sport Id ' + this.getSportId(this.state.formData.sportName)
      )
  ...
  componentDidMount() {
    ...
    ValidatorForm.addValidationRule('isText', (value) => {
      ...
      if (character != character.toUpperCase()) {
        this.setState({
          felMedelandeLeague: 'First letter should be upperCase',
        })
        return false
      }
      return true
